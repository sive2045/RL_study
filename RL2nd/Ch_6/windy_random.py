...
"""
Windy Gridworld with King's Moves with Stochastic (programming)
"""
...
# 1/3 : maintaing wind
# 1/3 : move one cell above existing
# 1/3 : move one cell below existing
WIND = [0, 0, 0, 1, 1, 1, 2, 2, 1, 0]
WIND_PS = [i + 1 for i in WIND]
WIND_SUB = [i - 1 for i in WIND]
...
def step(state, action):
    ...
    if action == ACTION_UP:
        ...
        return [min(max(i - 1 - WIND_[j], 0), WORLD_HEIGHT -1),  min(j + 1, WORLD_WIDTH - 1)]
    elif action == ACTION_LEFT_DOWN:
        return [max(min(i + 1 - WIND_[j], WORLD_HEIGHT - 1), 0), max(j - 1, 0)]
    elif action == ACCTION_RIGHT_DOWN:
        return [max(min(i + 1 - WIND_[j], WORLD_HEIGHT - 1), 0),  min(j + 1, WORLD_WIDTH - 1)]
    else: 
        assert False
...
def figure_6_3():
    ...
    plt.savefig(current_path + '/images/figure_6_3_2.png')
    plt.show()

...
if __name__ == "__main__":
    ...
    with open('real_data_trace.csv') as f:
        ncols = len(f.readline().split(','))

    data = np.loadtxt('real_data_trace.csv', delimiter=',', skiprows=1, usecols=range(1, ncols), dtype=np.int32)
    ...
    for e in range(EPISODES):
        ...
        if e == 0:
            ...
            for w in range(channel_size): # state 초기화 random select
...
#append / pop 배열에 넣고 빼기
    ...
    plt.figure(1)
    plt.plot(Loss, 'b')
    plt.savefig("./save_graph_comm/real_loss_dqn.png")

    plt.figure(2)
    plt.plot(episodes, scores, 'b')
    plt.savefig("./save_graph_comm/real_DMA_dqn.png")

    plt.figure(3)
    plt.bar(Numelement, total)
    plt.savefig("./save_graph_comm/real_distribution_dqn.png")

    if agent.load_model == True:
        agent.model.save_weights("./save_model_comm/real_DMA_dqn.h5")
        agent.model.save("./save_model_comm/real_DMA_model_dqn.h5")
    else:
        agent.model.save_weights("./save_model_comm/new_real_DMA_dqn.h5")
        agent.model.save("./save_model_comm/new_real_DMA_model_dqn.h5")
        #sys.exit()